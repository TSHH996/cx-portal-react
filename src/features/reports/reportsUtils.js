...
export function rptTicketToRow(ticket) {
  return {
    ...
    Brand: ticket.brand ?? "",
    "Complaint Date & Time": fmtDate(ticket.complaintAt),
    "Created By": ticket.createdByName ?? "",
    "Created By Email": ticket.createdByEmail ?? "",
    ...
    "Initial Action Taken": ticket.initialActionTaken ?? "No",
    "Initial Action Type": ticket.initialActionType ?? "",
    "Initial Customer Contact Status": ticket.initialCustomerContactStatus ?? "",
    "Initial Customer Satisfied": ticket.initialCustomerSatisfied ?? "",
    "Initial Resolution Details": ticket.initialResolutionDetails ?? "",
    "Resolution Action Type": ticket.resolutionActionType ?? "",
    "Customer Contact Status": ticket.customerContactStatus ?? "",
    "Customer Satisfied": ticket.customerSatisfied ?? "",
    "Resolution Date": ticket.resolutionDate ? fmtDate(ticket.resolutionDate) : "",
    "Handled By": ticket.resolutionHandledBy ?? "",
