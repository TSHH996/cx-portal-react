...
import { getLocalizedCategory, getLocalizedCity, getLocalizedSource } from "../../features/portal/newTicketConfig";
    ...
    <div className="rptFilterGroup rptFilterGroup-date">
      ...
      <div className="rptDateField">
        <button type="button" className={`rptDateTrigger${value ? " has-value" : ""}`} onClick={handleOpenPicker}>
          {value ? <bdi className="rptDateText data-value" dir="ltr">{value}</bdi> : <span className="rptDateText is-placeholder">{language === "ar" ? "اختر التاريخ" : "YYYY-MM-DD"}</span>}
...
function ReportsFiltersBar({ copy, filters, options, onChange, onReset, onQuickExport, language }) {
  ...
  return (
    <div className="rptFilterBar">
      ...
      <div className="rptFilterGroup"><label className="rptFilterLabel">{copy.rptBrand}</label><select className="rptSelect" dir="auto" value={filters.brand} onChange={(e) => onChange("brand", e.target.value)}><option value="all">{copy.insightsAllBrands}</option>{options.brands.map((value) => <option key={value} value={value}>{value}</option>)}</select></div>
      <div className="rptFilterGroup"><label className="rptFilterLabel">{copy.rptCity}</label><select className="rptSelect" dir="auto" value={filters.city} onChange={(e) => onChange("city", e.target.value)}><option value="all">{copy.rptAllCities}</option>{options.cities.map((value) => <option key={value} value={value}>{getLocalizedCity(value, language)}</option>)}</select></div>
      <div className="rptFilterGroup"><label className="rptFilterLabel">{copy.rptBranch}</label><select className="rptSelect" dir="auto" value={filters.branch} onChange={(e) => onChange("branch", e.target.value)}><option value="all">{copy.insightsAllBranches}</option>{branchOptions.map((value) => <option key={value} value={value}>{value}</option>)}</select></div>
      <div className="rptFilterGroup"><label className="rptFilterLabel">{copy.rptStatus}</label><select className="rptSelect" dir="auto" value={filters.status} onChange={(e) => onChange("status", e.target.value)}><option value="all">{copy.filterAllStatus}</option><option value="Open">Open</option><option value="In Progress">In Progress</option><option value="Replied">Replied</option><option value="Closed">Closed</option></select></div>
      <div className="rptFilterGroup"><label className="rptFilterLabel">{copy.rptSource}</label><select className="rptSelect" dir="auto" value={filters.source} onChange={(e) => onChange("source", e.target.value)}><option value="all">{copy.rptAllSources}</option>{options.sources.map((value) => <option key={value} value={value}>{getLocalizedSource(value, language)}</option>)}</select></div>
      <div className="rptFilterGroup"><label className="rptFilterLabel">{copy.rptPriority}</label><select className="rptSelect" dir="auto" value={filters.priority} onChange={(e) => onChange("priority", e.target.value)}><option value="all">{copy.filterAllPriority}</option><option value="High">High</option><option value="Medium">Medium</option><option value="Low">Low</option></select></div>
      <div className="rptFilterGroup"><label className="rptFilterLabel">{copy.rptSla}</label><select className="rptSelect" dir="auto" value={filters.slaStatus} onChange={(e) => onChange("slaStatus", e.target.value)}><option value="all">{copy.rptAllSla}</option><option value="pending">Pending</option><option value="at_risk">At Risk</option><option value="breached">Breached</option></select></div>
