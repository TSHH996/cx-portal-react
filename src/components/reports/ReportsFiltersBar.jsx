...
function ReportsDateField({ id, label, value, onChange }) {
    ...
    <div className="rptFilterGroup rptFilterGroup-date">
      ...
      <div className="rptDateField">
        <button type="button" className={`rptDateTrigger${value ? " has-value" : ""}`} onClick={handleOpenPicker}>
          <span className={`rptDateText${value ? "" : " is-placeholder"}`}>{value || "YYYY-MM-DD"}</span>
...
function ReportsFiltersBar({ copy, filters, options, onChange, onReset, onQuickExport }) {
  ...
  return (
    <div className="rptFilterBar">
      <ReportsDateField id="reports-date-from" label={copy.rptFrom} value={filters.dateFrom} onChange={(value) => onChange("dateFrom", value)} />
      <ReportsDateField id="reports-date-to" label={copy.rptTo} value={filters.dateTo} onChange={(value) => onChange("dateTo", value)} />
      ...
      <div className="rptFilterGroup"><label className="rptFilterLabel">{copy.rptCity}</label><select className="rptSelect" value={filters.city} onChange={(e) => onChange("city", e.target.value)}><option value="all">{copy.rptAllCities}</option>{options.cities.map((value) => <option key={value} value={value}>{value}</option>)}</select></div>
